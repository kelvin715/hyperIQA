...
def main(config):
    ...
    mse_all = np.zeros(config.train_test_num, dtype=float)
    r2_all = np.zeros(config.train_test_num, dtype=float)
    ...
    for i in range(config.train_test_num):
        ...
        solver = HyperIQASolver(config, path=None, train_idx=None, test_idx=None)
        srcc_all[i], plcc_all[i], mse_all[i], r2_all[i] = solver.train()
        
        # dataloader already split into train and test, so one time is enough

    # print(srcc_all)
    # print(plcc_all)
    srcc_med = np.median(srcc_all)
    plcc_med = np.median(plcc_all)
    mse_med = np.median(mse_all)
    r2_med = np.median(r2_all)

    print('Testing median SRCC %4.4f,\tmedian PLCC %4.4f ,\tmedian MSE %4.4f,\tmedian R2 %4.4f' % (srcc_med, plcc_med, mse_med, r2_med))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', dest='dataset', type=str, default='GC10_DET', help='Support datasets: livec|koniq-10k|bid|live|csiq|tid2013')
